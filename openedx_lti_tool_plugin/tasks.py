"""Tasks for openedx_lti_tool_plugin."""
import logging
...
from openedx_lti_tool_plugin.models import LtiGradedResource

log = logging.getLogger(__name__)


@shared_task(name='openedx_lti_tool_plugin.tasks.send_problem_score_update')
def send_problem_score_update(
    ...
    """
    for graded_resource in LtiGradedResource.objects.all_from_user_id(user_id=user_id, context_key=problem_id):
        log.info('LTI AGS: Sending AGS update for problem %s with user %s', problem_id, user_id)
...
@shared_task(name='openedx_lti_tool_plugin.tasks.send_vertical_score_update')
def send_vertical_score_update(
    ...
    Task to obtain a vertical's accumulated grade and update the AGS score asynchronously. This is a task that would be
    executed whenever a problem score is updated. We decided to do it this way because there is no way of telling if a
    score of a unit was changed.
    ...
    for graded_resource in vertical_graded_resources:
        log.info('LTI AGS: Sending AGS update for unit %s with user %s', str(vertical_key), user_id)
        graded_resource.update_score(
            earned,
            possible,
            datetime.now(tz=timezone.utc),
